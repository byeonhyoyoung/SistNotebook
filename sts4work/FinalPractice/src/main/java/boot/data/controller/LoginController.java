...
public class LoginController {
	...
	@Autowired
	MemberService service; //만든거가져오기(..MemberServiceInter,MemberService,MemberMapperInter)
	...
	public String loginprocess(@RequestParam String id,
			...
			HttpSession session,Model model) //400번오류..null값 체크 (required = false)해주기!!
	{
		...
		if(check==1) {
			...
			session.setAttribute("loginok", "yes"); //("yes")value값이므로 의미없음

...
<script type="text/javascript">
	function saveId(id){
		//alert(id); alert창에 뜬 id 부모창으로 입력하고 idSearch.jsp창 없애기
	...
	if(key==null){
		...
		<div style="margin: 10px 30px;">
			<form action="idSearch.jsp" method="post">
				...
				required="required" placeholder="아이디 입력"> <!-- rerequired(빈칸있으면안됨-->
	...
	<%}else{
		...
		<%}else{
			...
			<div style="margin: 50px 50px;">
				...
				<button type="button" class="btn btn-info" onclick="saveId('<%=m_id%>')">적용하기</button> <!-- 문자열 text ''소따옴표 해주기 -->
				<button type="button" class="btn btn-danger" onclick="location.href='idSearch.jsp'">다시입력</button>
			</div>
		<%}
	}
